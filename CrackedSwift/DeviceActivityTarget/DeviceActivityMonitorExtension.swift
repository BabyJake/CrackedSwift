...
class DeviceActivityMonitorExtension: DeviceActivityMonitor {
    ...
    override func eventDidReachThreshold(_ event: DeviceActivityEvent.Name, activity: DeviceActivityName) {
        ...
        guard activity.rawValue == focusActivityName else {
            print("[CrackedSwift] ⚠️ Device Activity extension (Target) — ignoring event for unknown activity: \(activity.rawValue)")
            return
        }

        // User used a monitored app → set violation flag and timestamp
        // Main app will only crack if it actually went to background and violation is recent
        print("[CrackedSwift] 🔴 LEFT APP (Screen Time): Device Activity extension (Target) — user used monitored app (event: \(event.rawValue)), setting violation flag")
